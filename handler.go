...
func PostBalasan(w http.ResponseWriter, r *http.Request) {
	...
	link := "https://medium.com/@kholiqnnabdul/whatsauth-free-2fa-otp-notif-whatsapp-gateway-api-gratis-be4dfb2f8434"
	if r.Header.Get("Secret") == os.Getenv("SECRET") {
		if msg.Message == "loc" || msg.Message == "Loc" || msg.Message == "lokasi" || msg.LiveLoc {
			...
			randm := []string{
				...
				"Jangan galak galak dong cuy, MAMAKKK AKU TAKUT",
				"Cie lagi mantau Bang Zay ya kamu, Masi bobo dia",
				"Jangan Lupa Bersyukur, biar gak Takabur,  CUSS AH KABURRRRR",
				"ALAMAKKKK",
			}
			dt := &wa.TextMessage{
				To:       msg.Phone_number,
...
func Liveloc(w http.ResponseWriter, r *http.Request) {
	...
	reply := fmt.Sprintf("cuyyy kamu pasti lagi di %s \n Koordinatenya : %s - %s\n", location,
